...
b.getPlayers(),c,e,d=a.length;d--;)for(c=a[d].getEntities(),e=c.length;e--;)if(!c[e].onBoard)return!1;return!0},"Player Has Entities On Nearby Connected Fields After Last Move":function(a){a=a.atomArguments;for(var c=b.gameWorld,e,d=b.getrecentlyMovedEntity().position,f,g,h,l=b.models.recentlyMovedEntity.playerID,k=b.models.GameGrid[d.y][d.x].cellID,m=a.length;m--;)for(f=a[m],g=f.length;g--;)if(e=f[g],d=!0,k===e){for(h=f.length;h--;)if(e=c.getFieldsWithFieldID(f[h],!0),0<e.contains.length){if(e.contains[0].playerID!==
l){d=!1;break}}else{d=!1;break}if(d)return!0}return!1},"Player Has No Movable Entities":function(a){a=b.getPlayerWithID(a.atomArguments);var c=a.countEntities(),e;if(0===c)return!0;for(e=c;e--;)if(!0===b.checkSelectCondition(a.getEntityFromArray(e))&&a.getEntityFromArray(e).canMove())return!1;for(e=c;e--;)if(!0===b.checkSelectCondition(a.getEntityFromArray(e))&&a.getEntityFromArray(e).canReachGoal())return!1;return!0},"Player Has Entities In Off Board Area":function(a){return b.offBoard.entitiesOfPlayerAreOutside(a.atomArguments)},
...
!0),n=p.position.x,p=p.position.y,!1!==d.getWorld().isEmpty(n,p)&&q.isLegalMove(d,a.conditions,e,k,n,p)&&(b=q.getUniqueMoveID(d.name,a.name,n,p),t.push({type:"MOVE",name:a.name,entity:d,targetX:n,targetY:p,moveClass:"move_standard",freq:c,ID:b,postMove:a.postMove}),q.addJobForMoveID({jobID:b,jobName:"move game entity",job:"Move Entity",jobArguments:{entity:d,destX:n,destY:p}}),a.postMove&&_.each(a.postMove,function(a){q.addJobForMoveID({jobID:b,jobName:a.jobName,job:a.jobFunction,jobArguments:a.jobArguments})}));
break;case "By Field ID":if("DICE"===a.frequency){m=q.getDiceBox().getDiceValues();n=m.length;p=[];u=0;e=-1;var r=!1;e=d.position.x;k=d.position.y;!1===_.isUndefined(d.tmp.fieldID)?(e=d.tmp.fieldID,r=!0):e=d.getWorld().getFieldID(e,k);for(k=n;k--;)"POSITIVE"===a.direction[1]?u=parseInt(e+m[k],10):"NEGATIVE"===a.direction[1]&&(u=parseInt(e-m[k],10)),"By Field ID"===a.type&&0===d.countOpponentsOnFieldsWithID(u,d)&&(p=d.getWorld().getFreeFieldsWithFieldID(u),p[0]?(n=p[0][0],p=12>=u?p[parseInt(p.length-
1,10)][1]:p[0][1],b=q.getUniqueMoveID(d.name,a.name,n,p),t.push({type:"DICE",diceID:k,diceValue:m[k],name:a.name,entity:d,targetX:n,targetY:p,moveClass:"move_standard",freq:"DICE",ID:b,postmove:a.postMove}),!0===r?(q.addJobForMoveID({jobID:b,jobName:"Put the entity to the destination field",job:"Place Entity",jobArguments:{entity:d,xPosition:n,yPosition:p}}),q.addJobForMoveID({jobID:b,jobName:"Remove the current entity from off board area",job:"Delete Entity from OffBoard",jobArguments:{entity:d}})):
q.addJobForMoveID({jobID:b,jobName:"move game entity",job:"Move Entity",jobArguments:{entity:d,destX:n,destY:p}}),q.addJobForMoveID({jobID:b,jobName:"Delete Dice Value",job:"Delete Dice Value",jobArguments:k}),a.postMove&&_.each(a.postMove,function(a){q.addJobForMoveID({jobID:b,jobName:a.jobName,job:a.jobFunction,jobArguments:a.jobArguments})})):d.countOwnOnFieldsWithID(u,d)===d.countFieldsWithID(u)&&(p=d.getWorld().getFieldsWithFieldID(u),p[0]&&(n=p[0].position.x,p=12>=u?p[0].position.y:p[parseInt(p.length-
1,10)].position.y,b=q.getUniqueMoveID(d.name,a.name,n,p),t.push({type:"DICE",diceValue:m[k],name:a.name,entity:d,targetX:n,targetY:p,moveClass:"move_standard",freq:"DICE",ID:b,postmove:a.postMove}),q.addJobForMoveID({jobID:b,jobName:"move game entity",job:"Move Entity",jobArguments:{entity:d,destX:n,destY:p}}),q.addJobForMoveID({jobID:b,jobName:"Delete Dice Value",job:"Delete Dice Value",jobArguments:k}),a.postMove&&_.each(a.postMove,function(a){q.addJobForMoveID({jobID:b,jobName:a.jobName,job:a.jobFunction,
jobArguments:a.jobArguments})}))))}}return t};d.canReachGoal=function(){return!_.isEmpty(d.getGoalMoves())};d.canCapture=function(){for(var a=d.getGoalMoves(),b=a.length;b--;)if("CAPTURE"===a[b].type)return!0;return!1};d.canCaptureAt=function(a,b){var c=d.getGoalMoves(),e,k;for(e=c.length;e--;)if(k=c[e],"CAPTURE"===k.type&&a===k.targetX&&b===k.targetY)return!0;return!1};d.getGoalMoves=function(a){var b,c,e,k=d.countGoals(),m,n;b=null;var p=[],t,q,v,u,r=d.getGame();t=a?a:d.getMoves();m=d.position.x;
n=d.position.y;if(!1===r.models.DiceBox.isEnabled){for(v=k;v--;)a=d.getGoal(v),!0===d.executeGoalByName(a.name)&&(b=d.translateDirection(a.move),c=parseInt(m+b[0],10),e=parseInt(n+b[1],10),b=r.getUniqueMoveID(d.name,a.name,c,e),p.push({ID:b,type:a.type,entity:d,name:a.name,preArrangeX:parseInt(m,10),preArrangeY:parseInt(n,10),targetX:c,targetY:e,moveClass:"move_goal"}));for(u=a=t.length;u--;)if(q=t[u],1<q.freq){r.doVirtualMove(d.position.x,d.position.y,q.targetX,q.targetY);m=q.targetX;n=q.targetY;
for(v=k;v--;)a=d.getGoal(v),a.move===q.name&&!0===d.executeGoalByName(a.name)&&(b=d.translateDirection(a.move),c=parseInt(m+b[0],10),e=parseInt(n+b[1],10),b=r.getUniqueMoveID(d.name,a.name,c,e),p.push({ID:b,type:a.type,entity:d,name:a.name,preArrangeX:parseInt(m,10),preArrangeY:parseInt(n,10),targetX:c,targetY:e,moveClass:"move_goal"}));r.undoVirtualMove()}}else for(t=r.getDiceBox().getDiceValues(),a=t.length,c=0,m=d.getWorld().getFieldID(m,n),!1===_.isUndefined(d.tmp.fieldID)&&(m=d.tmp.fieldID),
n=a;n--;)for(v=k;v--;)a=d.getGoal(v),!0===d.executeGoalByName(a.name,t[n])&&("BEAR OFF MOVE"===a.move?(q=a.area,b=r.getUniqueMoveID(d.name,a.name,q,"bearoff"),r.setMoveID(b),r.getDiceBox().connectMoveIDWithDiceValue(b,n,q),p.push({ID:b,type:"DICE",entity:d,name:a.name,diceID:n,diceValue:t[n],targetArea:q,moveClass:"move_bearoff"}),r.getArea(q).moveID=r.models.moveID):("MOVE POSITIVE"===a.move?c=parseInt(m+t[n],10):"MOVE NEGATIVE"===a.move&&(c=parseInt(m-t[n],10)),q=d.getWorld().getContentOfFieldsWithFieldID(c),
b=_.first(q),0===b&&(b=_.last(q)),q={x:b.position.x,y:b.position.y},b=r.getUniqueMoveID(d.name,a.name,q.x,q.y),r.setMoveID(b),r.getDiceBox().connectMoveIDWithDiceValue(b,n,q),p.push({ID:b,name:a.name,entity:d,type:"DICE",diceValue:t[n],diceID:n,targetX:parseInt(q.x,10),targetY:parseInt(q.y,10),moveClass:"move_goal"})));return p};d.canBeCaptured=function(){for(var a=d.getAssociatedPlayer().getNextOpponentPlayer(),b=a.countEntities(),a=a.getEntities();b--;)if(!0===a[b].canCaptureAt(d.position.x,d.position.y))return!0;
return!1};d.goals=[];d.assembleGoal=function(a){d.goals.push(a)};d.getGoal=function(a){return 0<=a&&a<d.goals.length?d.goals[a]:!1};d.countGoals=function(){return d.goals.length};d.executeGoalByName=function(a,b){if(!1===d.enabled)return!1;var c=d.goals.length,e;for(e=0;e<c;e+=1)if(d.goals[e].name===a){var c=!0,k=0;e=d.goals[e].atoms;for(var m=e.length,k=0;k<m;k+=1)if(c=d.getGame().executeEntityGoalAtomByName(d,e[k],b),!1===c||-1===c)return!1;return!0}return!1};d.goalAtoms=[];d.addGoalAtom=function(a,
b,c){d.goalAtoms.push({atomName:a,atomFunction:b,atomArguments:c})}}})();
